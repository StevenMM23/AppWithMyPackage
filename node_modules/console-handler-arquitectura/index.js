...
function getOptionsColor(colorName = "reset", button,
  spaceBetweenTitleButton = (title) => title,
  ...
  options.forEach(({ title, button: { name } }) => {
    consoleLogColor(
      colorName,
      `${functionTitle(title)} ${spaceBetweenTitleButton(title)} ${functionButton(name)}`
    );
  });
}
